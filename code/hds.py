...
class HDS:
    ...
    def buildGNMatrices(self, x, w):
        ...
        for t in range(self.T):
            ...
            for i in range(self.m):
                if t < self.T - 1:
                    gxt += coefs[t, i]*self.Qinv.dot(pt[t, i, :]) - coefs[t+1, i]*np.dot(np.transpose(dGs[t+1][i]),
                                                                                         self.Qinv.dot(pt[t+1, i, :]))
    ...
    def solveGNDirection(self, x, w, verbose=False):
        ...
        if verbose:
            print(np.linalg.norm(np.asarray(utils.multiplyMV(Ds, Ss, dx)) -
                                 np.asarray(gx_copy)), " ", np.linalg.norm(np.asarray(gx_copy)))
        return dx
